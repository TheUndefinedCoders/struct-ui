...
import { creativeComponents, basicBlocks } from '@/configs/leftSideBlocksMetaData';
...
function DocsSidebar() {
  ...
  const groupedComponents = creativeComponents && creativeComponents.reduce((acc, component) => {
    //@ts-ignore
    ...
    <aside className='h-full border-r'>
      <div className='sticky top-0 h-screen w-full rounded-md pt-[3.2em]'>
        <ScrollArea className='h-full py-4'>
          ...
          <h1 className='text-lg font-semibold pb-1'>Blocks</h1>
          {basicBlocks?.map((link: any) => {
            return (
              <>
                <li
                  key={link.href}
