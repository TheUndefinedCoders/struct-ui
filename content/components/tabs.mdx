export const metadata = {
  title: "Tabs - Animated Sliding Pill Tabs Component",
  description: "A modern, accessible React tabs component with smooth sliding pill indicator. Features animated transitions, dark mode support, and mobile-responsive design. Perfect for dashboard navigation, settings panels, and content organization with elegant hover effects and seamless tab switching.",
  keywords: [
    "Tabs React",
    "Animated Tabs React",
    "Moving indicator tabs",
    "Vercel style tabs",
    "Sliding pill indicator",
    "Active tab background",
    "Tabs with animation",
    "React animated navigation",
    "tab highlight",
    "Smooth tab transition",
    "UI tabs component",
    "Hover/active tab animation",
    "Accessible tabs React",
    "Interactive tab switcher",
    "Modern tabs UI",
    "Dark mode tabs",
    "Tailwind CSS tabs",
    "Pill navigation",
    "Settings navigation tabs",
    "Content switcher component",
  ],
  authors: [{ name: "Numan", url: "https://numan-dev.web.app" }],
  creator: "@numan-dev",
  publisher: "Struct-UI",
  robots: {
    index: true,
    follow: true,
    googleBot: {
      index: true,
      follow: true,
      "max-video-preview": -1,
      "max-image-preview": "large",
      "max-snippet": -1
    }
  },
  category: "technology"
};

```Tabs.tsx```
<ComponentCodePreview name='tabs' hasReTrigger isFitheight isDefaultCodeTab={true} />


## Two Tabs
<ComponentCodePreview name='tabs-two' hasReTrigger isFitheight />

## Three Tabs
<ComponentCodePreview name='tabs-three' hasReTrigger isFitheight />
